      ...
      <tr className="hover:bg-gray-700/50 transition-colors duration-150 group">
        <td className="px-4 py-3 align-top">
          <div className="flex items-center gap-2">
            ...
            title={item.announcementType} // show full text on hover
            className="flex items-center gap-2 text-sm text-gray-400 hover:text-cyan-400 transition-colors duration-150"
          >
            {/* text part: takes available width and truncates with … */}
            <span className="flex-1 min-w-0 truncate">
              {item.announcementType}
            </span>

            {/* icon stays visible and never shrinks */}
            <ExternalLinkIcon className="h-4 w-4 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
          </a>
        </td>
        <td className="px-4 py-3 align-top">
          {onSummaryClick ? (
